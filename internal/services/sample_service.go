// filepath: internal/services/sample_service.go
package services
...
type SampleService struct {
	DB                  *gorm.DB
	PrescriptionService *PrescriptionService // Inject PrescriptionService
}

// NewSampleService creates a new SampleService.
func NewSampleService(db *gorm.DB, ps *PrescriptionService) *SampleService {
...
func (s *SampleService) generateAutomaticPrescription(db *gorm.DB, examinationID uint, evaluationResult string) (models.Prescription, error) {
	...
	tempPrescriptionService := NewPrescriptionService(db) // Use the transaction DB
